...
def _get_other_modifiers(base_move: BaseMove, user: BattleMon, target: BattleMon, battle_state: BattleState) -> float:
    ...
    if target.abilities.has_ability("tangled_feet"):
        if status_repository.get("confusion") in target.status_conditions:
            modifier *= 0.5

    # Hustle 0.8 - if the attacker has it and it's a physical move
    if user.abilities.has_ability("hustle"):
        if base_move.damage_class == DamageClass.PHYSICAL:
            modifier *= 0.8
    ...
    if target.held_item and target.held_item.name == "bright_powder":
        modifier *= 0.9

    # Lax Incense 0.9 - if the target is holding it
    if target.held_item and target.held_item.name == "lax_incense":
        modifier *= 0.9

    # Wide Lens 1.1 - if the user is holding it
    if user.held_item and user.held_item.name == "wide_lens":
        modifier *= 1.1
...
def _get_micle_berry_modifier(user: BattleMon) -> float:
    ...
    if user.held_item and user.held_item.name == "micle_berry":
